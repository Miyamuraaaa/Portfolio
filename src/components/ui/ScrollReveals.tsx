    ...
    media.add("(prefers-reduced-motion: no-preference)", () => {
      scope.current?.querySelectorAll<HTMLElement>("[data-reveal-group]").forEach((group) => {
        ...
        const timeline = gsap.timeline({
          defaults: { ease: "none" },
          scrollTrigger: { trigger: group, start: `top+=${offset} 94%`, end: `clamp(top+=${offset} 64%)`, scrub: 0.35, invalidateOnRefresh: true },
        });
        const animate = (kind: string, from: gsap.TweenVars, at: number, duration: number) => {
          ...
          if (targets.length) timeline.fromTo(targets, from, { opacity: 1, y: 0, filter: "blur(0px)", scaleX: 1, duration }, at);
        };
